...
<head>
  ...
  <style>
     ...
     * Increased height to 250px as requested, 
     * plus overflow to handle taller content.
     */
    .navbar {
      position: sticky; /* keep the toolbar stuck at the top */
      top: 0;           /* offset from top */
      z-index: 1000;    /* ensure it stays above table rows/bars */
      background-color: #f8f9fa;
      padding: 0.25rem 0.5rem;
      width: 100%;
      height: 200px;    /* new fixed height of 200px for the toolbar */
      overflow-y: auto; /* scroll if the toolbar gets too tall */
    }
    ...
    /*
     * Collapsible area with a more horizontal layout (no wrapping),
     * plus a smaller gap for slimness.
     */
    .navbar-collapse {
      display: flex !important;
      flex-wrap: nowrap !important; /* force horizontal layout */
      gap: 0.5rem;                  /* smaller gap between tool groups */
    }

    /*
     * We'll adjust .toolbar-column to display in a row 
     * and align items center for a slimmer horizontal approach.
     */
    .toolbar-column {
      display: flex;
      flex-direction: row;
    ...
    /*
     * Kept .toolbar-group but left alone to avoid removing any functionality.
     * It can still hold labels and inputs in a column if needed.
     */
    .toolbar-group {
      display: flex;
    ...
    /*
     * Timescale block => two side-by-side columns for Year/Week input.
     */
    .timescale-row {
      display: flex;
      flex-direction: row;
   ...
   */
    ...
    /* 
     * Header cells => single-line text with ellipses 
     * (ensuring fonts fit on one line).
     */
    #ganttTable thead th {
      background-color: #f8f9fa;
      text-align: center;
    ...
    .timescale-header {
      ...
      min-width: 50px;
      /* The dynamic font size will be set inline in JS */
    }
    ...
    /*
     * We'll add a 'filler' row that remains at bottom
     */
    .filler-row {
      height: 40px;
    }
    ...
    /* 
     * Within the name cell, ensure single-line with ellipses for dynamic fitting.
     */
    .name-cell span {
      font-size: 0.85rem;
      white-space: nowrap;
    ...
    .bar-text {
      ...
      font-weight: 500;
      padding: 0 8px;
      /* The color will adjust in dark mode below */
    }
    ...
    /*
     * Overlap => diagonal stripes if bars truly overlap
     */
    .overlap-hatch {
      position: absolute;
      height: calc(var(--row-height) - 16px);
    ...
    /*
     * Filter modal => center, now taller (max-height 90vh)
     * and smaller font for less intrusiveness.
     */
    #filterModal {
      display: none;
      position: fixed;
      ...
      box-shadow: 0 8px 20px rgba(0, 0, 0, 0.5);
      z-index: 9999;
      min-width: 400px; /* reduced min-width to make it smaller */
      max-height: 90vh; /* increased max height to show more options */
      overflow: auto;
     ...
     * Updated to ensure all fonts become white (#fff) for better contrast.
     */
    body.dark-theme {
    ...
    body.dark-theme .name-cell {
      ...
      color: #fff; /* ensure white text in name cell */
      box-shadow: 1px 1px 3px rgba(255, 255, 255, 0.2);
    }
    body.dark-theme .bar-text {
      color: #fff; /* ensure bar label text is white */
    }
    ...
    body.dark-theme #csvEditModal {
      background: #343a40;
      color: #fff; /* ensure modal text is white */
    ...
    /*
     * Removed the vertical transform so the second slider is also horizontal
     */
    #heightZoomSlider {
      width: 120px; /* set to a standard width so it's horizontal now */
    }

    /*
     * Print => only prints the visible portion
     */
    @media print {
      .navbar,
      #barEditor,
    ...
    /*
     * Legend => items vertically, each with a color & label horizontally
     */
    ...
    /*
     * Updated toolbar arrangement:
     * Slicers remain horizontal as requested, but we add a max-height + scrolling
     * to accommodate the new 250px toolbar.
     */
    .slicers-horizontal {
      display: flex;
      flex-direction: row;
      ...
      max-height: 100%;  adjusted to fit within the 250px toolbar */
      overflow-y: auto;  /* scroll if slicing area grows too large */
    }
    /* Ensures the slicer labels remain fully visible */
    .slicer-group label {
      white-space: nowrap;
      overflow: visible;
      text-overflow: clip;
    ...
    /* 
     * Hide "Add Items" group by default. We'll toggle it in JS when adminMode is true.
     */
    #addItemsGroup {
      display: none; /* hidden by default, shown only in admin mode */
    }
  </style>
</head>
<body>
  ...
  <nav class="navbar navbar-expand-lg navbar-light bg-light">
    <div class="container-fluid">
      ...
      >
        ...
        <div class="toolbar-column">
          <div class="toolbar-group">
            ...
            <div style="display: flex; flex-direction: row; gap: 0.5rem;">
              <div>
                <!-- H-Zoom slider min set to 10 for narrower columns -->
              ...
              <div>
                <!-- The "vertical" slider is now also horizontal, removing rotation -->
          ...
          <div class="toolbar-group">
            ...
            />
            <button id="filterBtn" class="btn btn-outline-primary btn-sm mt-1">
              Filter
            </button>
          </div>
        </div>
        ...
        <div class="toolbar-column">
          <!-- Hidden by default; only shown in admin mode below -->
          <div id="addItemsGroup" class="toolbar-group">
            <span>Add Items</span>
            <button id="addLineItemBtn" class="btn btn-primary btn-sm">
              Add Line
            </button>
            <button id="addBarBtn" class="btn btn-secondary btn-sm">
              Add Bar
            </button>
          </div>
          <div class="toolbar-group">
            <span>History</span>
            <button id="undoBtn" type="button" class="btn btn-sm btn-success">
              Undo
            </button>
            <button id="redoBtn" type="button" class="btn btn-sm btn-warning">
              Redo
            </button>
          </div>
        </div>

        <!-- Column 4 => Legend vertically stacked -->
        <div class="toolbar-column">
        ...
        <div class="toolbar-column">
          <div class="toolbar-group">
            ...
            <button id="importCsvBtn" class="btn btn-outline-success btn-sm">
              Import CSV
            </button>
            <button id="downloadCsvBtn" class="btn btn-outline-secondary btn-sm">
              Export CSV
            </button>
            <button id="printBtn" class="btn btn-outline-dark btn-sm">
              Print/PDF
            </button>
          </div>
        </div>
        ...
        <div class="toolbar-column">
          <div class="toolbar-group">
            <span>Misc</span>
            <button id="adminToggleBtn" class="btn btn-sm btn-outline-danger">
              Admin
            </button>
            <button id="themeToggleBtn" class="btn btn-sm btn-outline-info">
              Theme
            </button>
            <button id="aboutToggleBtn" class="btn btn-sm btn-outline-secondary">
              About
            </button>
            <button
              id="csvEditToggleBtn"
        ...
        <!-- Column 7 => Slicers arranged horizontally -->
        <div class="toolbar-column">
          <div class="slicers-horizontal">
            <!-- discipline slicer: multiple -->
            ...
            <!-- client slicer: multiple -->
            <div class="slicer-group">
              <label>Client</label>
              <button id="clearClientSlicerBtn" class="btn btn-sm btn-warning">
                Clear
              </button>
              <select
  ...
  <div id="barEditor">
    ...
    <div id="barEditorActions">
      ...
      <button id="barEditorCancelBtn" class="btn btn-sm btn-secondary">
        Cancel
      </button>
    </div>
  </div>
  ...
  <div id="lineEditor">
    ...
    <div id="lineEditorActions">
      <button id="lineDeleteBtn" class="btn btn-sm btn-danger">
        Delete Line Item
      </button>
      <button id="lineEditorOkBtn" class="btn btn-sm btn-primary">OK</button>
      <button id="lineEditorCancelBtn" class="btn btn-sm btn-secondary">
        Cancel
      </button>
    </div>
  </div>
  ...
  <div id="filterModal">
    ...
    <div class="filter-container">
      <!-- Left filter card => line item fields as dropdowns -->
      ...
      <!-- Right filter card => bar attributes as dropdowns -->
      <div class="filter-card">
        <h5>Bar Filters</h5>
    ...
    <div id="filterActions">
      ...
      <button id="filterClearBtn" class="btn btn-sm btn-secondary">
        Clear
      </button>
      <button id="filterCancelBtn" class="btn btn-sm btn-secondary">
        Cancel
      </button>
    </div>
  </div>
  ...
  <div id="settingsModal">
    ...
    <div id="settingsActions">
      ...
      <button id="settingsCancelBtn" class="btn btn-sm btn-secondary">
        Cancel
      </button>
    </div>
  </div>
  ...
  <div id="csvEditModal">
    ...
    <div id="csvEditModalActions">
      <button id="csvEditApplyBtn" class="btn btn-sm btn-primary">
        Apply Changes
      </button>
      <button id="csvEditCancelBtn" class="btn btn-sm btn-secondary">
        Close
      </button>
    </div>
  </div>
  ...
  <script>
    /* 
      ...
      We will call pushState() whenever the user moves bars/lines so that Undo/Redo covers:
      - moving bars up/down
      - moving line items up/down
      - moving bars left/right
    */
    let adminMode = false; // Admin toggle
    let typeColors = {
      secured: "#28a745",
      proposed: "#fd7e14",
      opportunity: "#dc3545",
    ...
    /* 
      lines: Each line has an array of bars, plus line-level attributes 
      like name, client, discipline, etc.
    */
    let lines = [
      {
        id: 1,
    ...
    // Slicer-based filters => discipline, client, experience, location
    let slicers = {
      discipline: null,
      client: null,
      experience: null,
    ...
    /*
      ...
      TIME & HELPER 
      ---------------------------------------------------------------------------------------------
      Convert year/week to absolute week or vice versa.
    */
    ...
    /*
      ...
      LOCAL STORAGE 
      ---------------------------------------------------------------------------------------------
      Save/Load entire chart configuration to localStorage.
    */
    ...
    /*
      ...
      THEME & CSS
      ---------------------------------------------------------------------------------------------
      Update legend swatches & apply theme changes.
    */
    function updateLegendSwatches() {
      document.getElementById("legendSecuredSwatch").style.backgroundColor =
        typeColors.secured;
      document.getElementById("legendProposedSwatch").style.backgroundColor =
        typeColors.proposed;
      document.getElementById("legendOpportunitySwatch").style.backgroundColor =
        typeColors.opportunity;
      document.getElementById("legendFutureSwatch").style.backgroundColor =
        typeColors.future;
    }
    function applyTheme() {
    ...
    */
    function updateBarTextPositions() {
      ...
      document.querySelectorAll(".bar-text").forEach((textEl) => {
        ...
        let textLeft =
          visibleLeft - barLeft + (visibleWidth - textWidth) / 2;
    ...
    /*
      ---------------------------------------------------------------------------------------------
      renderHeader => timescale columns with dynamic font-size based on cellWidth.
      ---------------------------------------------------------------------------------------------
    */
    function renderHeader() {
      ...
      for (let i = 0; i < totalWeeks; i++) {
        ...
        // NEW CODE: dynamic font size scaling for timescale headers
        // so text is still visible whether column is wide or narrow
        let dynamicFontSize = Math.max(8, Math.min(16, cellWidth * 0.2));
        th.style.fontSize = dynamicFontSize + "px";
    ...
    */
    ...
    /*
      ...
      renderTable => main UI
      ---------------------------------------------------------------------------------------------
      Key logic: line must pass line filters, bar must pass bar filters, slicers must pass. 
    */
    function renderTable() {
      ...
      let filteredLines = lines.filter((line) => {
        ...
        if (Object.keys(filters.line).length > 0) {
          for (let f in filters.line) {
            ...
            if (
              !line[f] ||
              !line[f].toLowerCase().includes(filterValue)
            ) {
              return false;
            }
          }
        ...
        // 2) slicers => discipline, client, experience, location
        if (slicers.discipline) {
          if (Array.isArray(slicers.discipline)) {
            ...
            if (!selLowerArr.includes(discLower)) {
              return false;
            }
          } else {
            if (
              line.discipline.toLowerCase() !==
              slicers.discipline.toLowerCase()
            ) {
              return false;
            }
          }
          ...
          if (Array.isArray(slicers.client)) {
            ...
            for (let cVal of slicers.client.map(c => c.toLowerCase())) {
              let lineMatchesClient =
                line.client &&
                line.client.toLowerCase() === cVal;
              let barHasClient = line.bars.some(
                (b) =>
                  b.client &&
                  b.client.toLowerCase() === cVal
              );
            ...
            const lineMatchesClient =
              line.client &&
              line.client.toLowerCase() === slicers.client.toLowerCase();
            const barHasClient = line.bars.some(
              (b) =>
                b.client &&
                b.client.toLowerCase() === slicers.client.toLowerCase()
            );
            if (!lineMatchesClient && !barHasClient) {
              return false;
            }
        ...
        if (slicers.experience) {
          if (Array.isArray(slicers.experience)) {
            ...
            if (
              line.experience.toLowerCase() !==
              slicers.experience.toLowerCase()
            ) {
              return false;
            }
          }
        ...
        if (slicers.location) {
          if (Array.isArray(slicers.location)) {
            ...
            if (
              line.officeLocation.toLowerCase() !==
              slicers.location.toLowerCase()
            ) {
              return false;
            }
          }
        ...
        // 3) bar filter => at least one bar passes if bar filters exist
        ...
        // 4) search => line or any bar
        if (lowerSearch) {
          let lineMatch =
            (line.name &&
              line.name.toLowerCase().includes(lowerSearch)) ||
            (line.client &&
              line.client.toLowerCase().includes(lowerSearch)) ||
            (line.discipline &&
              line.discipline.toLowerCase().includes(lowerSearch)) ||
            (line.experience &&
              line.experience.toLowerCase().includes(lowerSearch)) ||
            (line.officeLocation &&
              line.officeLocation.toLowerCase().includes(lowerSearch)) ||
            (line.geoLocation &&
              line.geoLocation.toLowerCase().includes(lowerSearch)) ||
      ...
      filteredLines.forEach((line, lineIndex) => {
        ...
        barContainer.addEventListener("contextmenu", (ev) => {
          if (adminMode) {
            // Right-click in empty area => add new bar if admin
            if (
              ...
              line.bars.push(nb);
              pushState(); // Ensure adding a bar can be undone
              renderTable();
              saveToLocalStorage();
            }
          }
        ...
        // Filter bars that pass bar filters
        ...
        // Overlap => create diagonal stripes if bars overlap
        let sorted = [...finalBarsToRender].sort(
          (a, b) => a.startAbsWeek - b.startAbsWeek
        );
        for (let i = 0; i < sorted.length; i++) {
          for (let j = i + 1; j < sorted.length; j++) {
            ...
            if (overlapE > overlapS) {
              let leftPx =
                (Math.max(displayStart, overlapS) - displayStart) *
                cellWidth;
              let wPx =
                (Math.min(displayEnd + 1, overlapE) -
                  Math.max(displayStart, overlapS)) *
                cellWidth;
              if (wPx > 0) {
    ...
    /*
      ---------------------------------------------------------------------------------------------
      onLineMouseDown => reorder lines by dragging
      ---------------------------------------------------------------------------------------------
      We capture the state at mouseup in the global mouseup event.
    */
    ...
    /*
      ---------------------------------------------------------------------------------------------
      onBarMouseDown => bar drag/move
      ---------------------------------------------------------------------------------------------
      We capture the final state in the global mouseup event for undo/redo.
    */
    ...
    /*
      ---------------------------------------------------------------------------------------------
      MOUSEMOVE => handle dragging (bars or lines)
      ---------------------------------------------------------------------------------------------
      On each mousemove, we update the relevant item in memory, then re-render.
      pushState() is not called until mouseup to capture the final state for undo/redo.
    */
    document.addEventListener("mousemove", (e) => {
      ...
      if (dragType === "line-reorder" && draggingLine) {
        ...
        if (newIndex >= mainFilteredLines.length)
          newIndex = mainFilteredLines.length - 1;
        ...
        if (newIdx >= mainFilteredLines.length)
          newIdx = mainFilteredLines.length - 1;
        if (newIdx !== idxCur) {
          currentLine.bars = currentLine.bars.filter(
        ...
        if (newIndex !== oldLineIdx) {
          mainFilteredLines[oldLineIdx].bars = mainFilteredLines[
            oldLineIdx
          ].bars.filter((b) => b.id !== currentBar.id);
    ...
    /*
      ...
      ---------------------------------------------------------------------------------------------
      We call pushState() here so that Undo/Redo can revert or reapply these movements.
    */
    document.addEventListener("mouseup", () => {
      ...
      if (dragType === "line-reorder") {
        ...
        draggingLine = null;
        pushState(); // Save final arrangement to undo stack
        saveToLocalStorage();
        renderTable();
      } else if (
        [
        ...
        currentBar = null;
        currentLine = null;
        pushState(); // Save final bar movement to undo stack
        saveToLocalStorage();
      }
      dragType = null;
    });
    ...
    */
    ...
    /*
      ...
      ---------------------------------------------------------------------------------------------
      The editor is shown on right-click of a bar. We also auto-save if admin changes fields.
    */
    function showBarEditor(bar, line, x, y) {
      ...
      let { year: ey, week: ew } = fromAbsoluteWeek(
        bar.startAbsWeek + bar.length - 1
      );
      ...
      setTimeout(() => {
        ...
        if (rect.right > window.innerWidth) {
          barEditor.style.left =
            Math.max(0, window.innerWidth - rect.width - 10) + "px";
        }
        if (rect.bottom > window.innerHeight) {
          barEditor.style.top =
            Math.max(0, window.innerHeight - rect.height - 10) + "px";
        }
      }, 0);
      ...
      const autosave = () => {
        ...
        currentEditorBar.length =
          toAbsoluteWeek(eY, eW) - currentEditorBar.startAbsWeek + 1;
    ...
    barEditorOkBtn.addEventListener("click", () => {
      ...
      pushState(); // finalize changes for undo
    });
    barEditorCancelBtn.addEventListener("click", () => {
      barEditor.style.display = "none";
      currentEditorBar = null;
      currentEditorLine = null;
      pushState(); // just in case any changes occurred
    });
    barDeleteBtn.addEventListener("click", () => {
      if (!adminMode) return;
      if (!currentEditorBar || !currentEditorLine) return;
      currentEditorLine.bars = currentEditorLine.bars.filter(
        (b) => b.id !== currentEditorBar.id
      );
      barEditor.style.display = "none";
      currentEditorBar = null;
      currentEditorLine = null;
    ...
    /*
      ...
      ---------------------------------------------------------------------------------------------
      The editor is shown on right-click of a line name cell.
    */
    function showLineEditor(line, x, y) {
      ...
      setTimeout(() => {
        ...
        if (rect.right > window.innerWidth) {
          lineEditor.style.left =
            Math.max(0, window.innerWidth - rect.width - 10) + "px";
        }
        if (rect.bottom > window.innerHeight) {
          lineEditor.style.top =
            Math.max(0, window.innerHeight - rect.height - 10) + "px";
        }
      }, 0);
      ...
      const autosaveLine = () => {
        ...
        currentEditorLine.officeLocation =
          lineOfficeLocationInput.value;
    ...
    /*
      ...
      ---------------------------------------------------------------------------------------------
      Now multi-select is handled by reading selectedOptions into arrays. 
    */
    const slicerDisciplineSelect = document.getElementById(
      "slicerDisciplineSelect"
    );
    const slicerClientSelect = document.getElementById("slicerClientSelect");
    const slicerExperienceSelect = document.getElementById(
      "slicerExperienceSelect"
    );
    const slicerLocationSelect = document.getElementById(
      "slicerLocationSelect"
    );

    document
      .getElementById("clearDisciplineSlicerBtn")
    ...
    document
      .getElementById("clearClientSlicerBtn")
      .addEventListener("click", () => {
        slicers.client = null;
    ...
    document
      .getElementById("clearExperienceSlicerBtn")
      .addEventListener("click", () => {
    ...
    document
      .getElementById("clearLocationSlicerBtn")
      .addEventListener("click", () => {
    ...
    // On slicer selects: build arrays
    ...
    // Populate options in slicers from lines data
    function populateSlicers() {
      ...
      lines.forEach((line) => {
        ...
        // Also check bar client for the client slicer
        line.bars.forEach((b) => {
    ...
    /*
      ---------------------------------------------------------------------------------------------
      DYNAMIC FILTER => build from data for line & bar
      ---------------------------------------------------------------------------------------------
      The user can open a filter modal, choose from dropdowns, then apply or clear.
    */
    ...
    const filterLineDiscipline = document.getElementById(
      "filterLineDiscipline"
    );
    const filterLineExperience = document.getElementById(
      "filterLineExperience"
    );
    const filterLineOfficeLocation = document.getElementById(
      "filterLineOfficeLocation"
    );
    const filterLineGeoLocation = document.getElementById(
    ...
    function autoRefreshFilters() {
      ...
      filters.line.officeLocation =
        filterLineOfficeLocation.value || null;
    ...
    /*
      ...
      FILTER modal
      ---------------------------------------------------------------------------------------------
      Closing when user clicks outside the modal is handled below.
    */
    filterBtn.addEventListener("click", () => {
      ...
      filterLineOfficeLocation.value =
        filters.line.officeLocation || "";
    ...
    */
    document.getElementById("aboutToggleBtn").addEventListener("click", () => {
      alert(
        ` Sleek Professional Gantt Planner - Overview 
• Timescale config, quick ranges, H/V zoom, search & filter. 
• Slicers for discipline/client/experience/location, stacked horizontally & multi-select. 
• “Client” slicer checks line.client OR any bar.client. 
• Admin mode => advanced drag editing, CSV in-browser. 
• Local storage, import/export CSV, print PDF, undo/redo. 
• Legend auto-updates with theme colors. 
Enjoy planning!
`
      );
    });
    ...
    */
    ...
    /*
      ...
      ---------------------------------------------------------------------------------------------
      If user enters correct password, we enable admin mode. 
    */
    ...
    */
    ...
    csvEditToggleBtn.addEventListener("click", () => {
      ...
      lines.forEach((line) => {
        if (!line.bars.length) {
          csvStr += `${line.id || ""},"${line.name || ""}","${line.client || ""}","${
            line.discipline || ""
          }","${line.experience || ""}","${line.officeLocation || ""}","${
            line.geoLocation || ""
          }","${line.type || ""}",,,,,,,,,,,,\n`;
        } else {
          line.bars.forEach((bar) => {
            ...
            csvStr += `${
              bar.id || ""
            },"${bar.label || ""}","${bar.projectName || ""}","${
              bar.client || ""
            }","${bar.discipline || ""}","${
              bar.experience || ""
            }","${bar.LOE || ""}","${bar.location || ""}","${
              bar.assignedTo || ""
            }","${bar.proposed || ""}","${bar.type || ""}",`;
    ...
    */
    ...
    downloadCsvBtn.addEventListener("click", () => {
      ...
      lines.forEach((line) => {
        if (!line.bars.length) {
          csv += `${line.id || ""},"${line.name || ""}","${line.client || ""}","${
            line.discipline || ""
          }","${line.experience || ""}","${line.officeLocation || ""}","${
            line.geoLocation || ""
          }","${line.type || ""}",,,,,,,,,,,,\n`;
    ...
    */
    ...
    /*
      ...
      ---------------------------------------------------------------------------------------------
      These alive controls let users see more or fewer weeks (H-zoom) & bigger or smaller row height.
      pushState() not strictly needed here as it's a visual setting, but we store state in localStorage.
    */
    ...
    /*
      ...
      UNDO/REDO 
      ---------------------------------------------------------------------------------------------
      pushState() is called after user actions that change the data (e.g., dragging, adding, etc.).
      This ensures we can revert bar/line movement or addition.
    */
    function pushState() {
      let st = JSON.parse(
        JSON.stringify({
    ...
    function undo() {
      if (!adminMode) return; // only in admin mode
    ...
    */
    ...
    /*
      ...
      INIT => onload
      ---------------------------------------------------------------------------------------------
      We load from local storage, set up, and do the first render.
    */
    ...
    function init() {
      ...
      // Start with admin mode off => hide "Add Items"
    ...
    /*
     * parseCsv => minimal CSV parser that accounts for quoted fields
     */
    function parseCsv(csvString) {
      let linesArr = csvString.split(/\r?\n/);
      if (!linesArr.length)
        return { lines: [], lineCounter: 0, barCounter: 0 };
      ...
      data.forEach((r) => {
        ...
        if (!rebuilt[lineId]) {
          rebuilt[lineId] = {
            id:
              parseInt(lineId) ||
              1000 + Object.keys(rebuilt).length,
            ...
            officeLocation: stripQuotes(
              r["Line OfficeLocation"] || ""
            ),
